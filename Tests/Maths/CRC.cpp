...
/*const std::array<uint32_t, 256> expected_lookup = { 0, 0x77073096, 0xEE0E612C, 0x990951BA, 0x076DC419, 0x706AF48F,
...
template<Stf::Concepts::CRCDescription T> void run_check_test() {
    ...
    for (const char c : std::string { "123456789" }) {
        ...
        with_lookup.update(c);
    }

    const auto expected = T::check;

    fmt::print("Testing {}\n", T::name);
    ASSERT_EQ(without_lookup.finished_value(), expected);
